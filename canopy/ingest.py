"""RentCast ingest -- restored as a background-only weekly feed
(canopy/cli.py::run_rentcast_weekly), not the primary listings source.
RentCast-sourced rows never enter the rating queue (see
canopy.rating.excluded_listing_ids) -- their only purpose is to stay
fresh enough for canopy/rentcast_backfill.py's address-matching to find
data Zillow's alert emails don't carry (lot size, year built, property
type, MLS info). See docs/ARCHITECTURE.md's Appendix for why RentCast
was retired as the primary source in the first place."""
